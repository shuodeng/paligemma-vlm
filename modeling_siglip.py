...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.patch_embedding = nn.Conv2d(
            ...
            stride=self.patch_size, # patches doesn't overlap
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        # since the stride is equal to the kernel size
...
class SiglipEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        residual = hidden_states # save the initial hidden_states value
        # [Batch_Size, Num_Patches, Embed_Dim] -> [Batch_Size, Num_Patches, Embed_Dim]
        hidden_states = self.layer_norm1(hidden_states)
        # [Batch_Size, Num_Patches, Embed_Dim] -> [Batch_Size, Num_Patches, Embed_Dim]
        hidden_states, _ = self.self_attn(hidden_states=hidden_states)
        # [Batch_Size, Num_Patches, Embed_Dim]
        hidden_states = residual + hidden_states # add back residual
        # residual: [Batch_Size, Num_Patches, Embed_Dim]
...
class SiglipVisionTransformer(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.encoder = SiglipEncoder(config) # an encoder only attension block
